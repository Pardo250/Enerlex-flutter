...
class _ConfigScreenState extends State<ConfigScreen> {
  ...
  Widget _buildProfileCard(BuildContext context, AppState user, Color cardBg, Color textPrimary, Color textMuted) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            CircleAvatar(radius: 30, backgroundColor: _accent, child: Text(user.inicial, style: const TextStyle(color: Colors.black, fontSize: 24, fontWeight: FontWeight.w700))),
            const SizedBox(width: 14),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(user.nombre.isNotEmpty ? user.nombre : 'Usuario', style: TextStyle(color: textPrimary, fontSize: 16, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 2),
                  Text(user.correo.isNotEmpty ? user.correo : '', style: TextStyle(color: textMuted, fontSize: 13)),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          _buildToggleItem('Notificaciones', 'Push, Email, SMS', textPrimary, textMuted, _notificationsOn, (val) => setState(() => _notificationsOn = val)),
          _buildDivider(isDark),
          _buildNavItem('Dispositivos', '${user.dispositivos.length} vinculados', textPrimary, textMuted, () {}),
          _buildDivider(isDark),
          _buildToggleItem('Modo oscuro', isDark ? 'Activado' : 'Desactivado', textPrimary, textMuted, isDark, (val) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Transform.scale(
            scale: 0.85,
            child: Switch(value: value, onChanged: onChanged, activeColor: Colors.white, activeTrackColor: _accent, inactiveThumbColor: Colors.white, inactiveTrackColor: const Color(0xFF2E3348), trackOutlineColor: WidgetStateProperty.all(Colors.transparent)),
          ),
        ],
      ),
    );
  ...
  Widget _buildDivider(bool isDark) => Divider(height: 1, thickness: 1, color: isDark ? Colors.white.withOpacity(0.05) : Colors.black.withOpacity(0.06), indent: 16, endIndent: 16);

  Widget _buildLogoutButton(Color cardBg) {
    return Container(
      width: double.infinity,
      decoration: BoxDecoration(color: cardBg, borderRadius: BorderRadius.circular(16), border: Border.all(color: _redColor.withOpacity(0.3), width: 1)),
